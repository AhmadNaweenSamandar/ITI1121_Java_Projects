...
public class Simulator {
	...
	public void simulate() {
		...
		while (clock < steps) {
			...
			// Determine if a car arrive in the second
			if (RandomGenerator.eventOccurred(probabilityOfArrivalPerSec)){
				Car c = RandomGenerator.generateRandomCar();
				//Creating a new spots
				Spot arrivingSpot = new Spot(null, 0);
				arrivingSpot.setCar(c);
				arrivingSpot.setTimestamp(clock);
				// putting it in the queue
				incomingQueue.enqueue(arrivingSpot);
			}
			...
			for (int i = 0; i < lot.getNumRows(); i++){
				for (int j = 0; j < lot.getNumSpotsPerRow(); j++){
					...
					if (spot != null && spot.getCar() != null){
						...
						//Calculate probability of departure using traingle distribution
						Rational departureProbability = departurePDF.pdf(duration);
						if (RandomGenerator.eventOccurred(departureProbability) || duration > MAX_PARKING_DURATION){
							// enqueue the spot for departure
							Car departingCar = spot.getCar();
							outgoingQueue.enqueue(new Spot(departingCar, spot.getTimestamp()));
			...
			if (waitingSpot == null && !incomingQueue.isEmpty()){
				waitingSpot = incomingQueue.dequeue();
			}
			
			if (waitingSpot != null){
				//using ParkingLot file attemptParking method
				if (lot.attemptParking(waitingSpot.getCar(), clock)){
					// printing record of entry 
					System.out.println(waitingSpot.getCar() + " ENTERED at timestep " + clock + " occupancy is at " + lot.getTotalOccupancy());
					// reseting waitingSpot
					waitingSpot = null;
				}
			}
			
			
			// dispatcing one car from outgoing queue
			if (!outgoingQueue.isEmpty()){
				Spot departingSpot = outgoingQueue.dequeue();
				System.out.println(departingSpot.getCar() + " EXITED at timestep " + clock + "; occupancy is at " + lot.getTotalOccupancy());
			}
